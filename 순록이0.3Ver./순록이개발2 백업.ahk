...
if not A_IsAdmin
{
	MSGBOX, 262160, 실행오류, "관리자권한으로실행"으로 실행해주세요.
	ExitApp
	return
}
...
Gui, Add, Button, x15 y140 w120 h40 gStop_this, 순록이 종료 ; 순록이 자동사냥 종료
...
ATT1(){
	gui, submit, nohide
	GuiControl,, A, 순록이 제거중...
	Sleep, 100
	TargetTitle := ElanciaTitle
	WinGet, TargetPID, PID, %TargetTitle% 
	wall_remove_enable()
	ride_enable()
	floor_remove_enable()
	char_remove_enable()
